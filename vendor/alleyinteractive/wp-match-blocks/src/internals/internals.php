 ...
 *
 * @param mixed[] $args 'attrs' argument.
 * @return ValidatorInterface
 */
function parse_attrs_clauses( array $args ): ValidatorInterface {
	$relation = 'AND';

	if ( isset( $args['relation'] ) && is_string( $args['relation'] ) && 'OR' === strtoupper( $args['relation'] ) ) {
		$relation = 'OR';
	}
...
function parse_nth_of_type( $args, int $max ): array {
	...
	foreach ( $selectors as $selector ) {
		...
		if ( is_string( $selector ) && preg_match( '/^([+-]?\d+)?(-?n)?([+-]\d+)?$/', $selector, $pieces ) ) {
