...
final class Block_Attribute extends Block_Validator {
	...
	public function setOptions( $options = [] ) {
		...
		if ( null !== $next['key'] ) {
			try {
				...
				throw new InvalidArgumentException( esc_html( 'Invalid clause for attribute key: ' . $exception->getMessage() ) );
			}
		}
		...
		if ( self::UNDEFINED !== $next['value'] ) {
			try {
				...
				throw new InvalidArgumentException( esc_html( 'Invalid clause for attribute value: ' . $exception->getMessage() ) );
			}
		}
	 ...
	 */
	protected function setVal( $val ): void {
	 ...
	 * @param mixed[]            $values    Indexed list of values to test.
	 * @return mixed[] The indices of $values that passed the comparison and the
	 ...
	 *
	 * @param mixed[] $tests  Zero or more arrays, each containing the indices of
	...
	private static function passing( array $tests, array $misses ): bool {
		return (
			(
				...
				|| (
					\count( array_merge( ...$tests ) ) > 0 // @phpstan-ignore argument.type
					|| array_filter( $misses )
				)
			)
			&& (
				...
				(
					\count( $tests ) > 1
					&& array_intersect( ...$tests ) // @phpstan-ignore argument.type
				)
				// Either key or value was tested, and at least one index matched.
				|| (
					\count( $tests ) === 1
					&& \count( ...$tests ) > 0 // @phpstan-ignore greater.alwaysTrue, argument.type
				)
